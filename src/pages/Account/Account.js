...
function Account() {
  ...
  return (
    <div className="container">
      ...
      {(extensionIsSelected && !connectingWallet) &&
        <MainBlock
          class="account"
          smallTitle={true}
          normalTitle={true}
          ...
          content={
            <div className="account-body">
              ...
              <div className="account-wrapper">
                ...
                <button className="account-btn account-disconnect" onClick={() => disconnectHandler()}>Disconnect</button>
              </div>
            </div>
          }
          footer={
                ...
                <ul className="account-footer-list">
                  { transactionsList.map((i, index) => i.type === "swap" && (
                    <li className="account-footer-list-item" key={index}>
                      {i.toValue === null && <span>Swapping {i.fromValue.toFixed(4)} {i.fromSymbol} to {i.toSymbol}</span>}
                      {i.toValue !== null && <span>Swap {i.fromValue.toFixed(4)} {i.fromSymbol} to {i.toValue.toFixed(4)} {i.toSymbol}</span>}
